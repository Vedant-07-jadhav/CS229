...
def backward_relu(x, grad_outputs):
    ...
    # *** START CODE HERE ***
    grad_relu = np.where(x > 0, 1, 0)  # ReLU gradient
    return np.array(grad_relu * grad_outputs)
...
def backward_convolution(conv_W, conv_b, data, output_grad):
    ...
    # *** START CODE HERE ***
    conv_channels, input_channels, conv_width, conv_height = conv_W.shape
    _, input_width, input_height = data.shape
    
    # Get the actual output_grad dimensions
    ...
    # Gradient w.r.t bias: sum the output gradient for each output channel
    for out_c in range(conv_channels):
        grad_b[out_c] = np.sum(output_grad[out_c])
    ...
    for x in range(out_width):
        for y in range(out_height):
            for out_c in range(conv_channels):
                ...
                grad_W[out_c] += output_grad[out_c, x, y] * data[:, x:x+conv_width, y:y+conv_height]
    ...
    for i in range(input_channels):
        for x in range(0, input_width, pool_width):
            for y in range(0, input_height, pool_height):
                ...
                # Find the position of max value in the window
                max_idx = np.unravel_index(window.argmax(), window.shape)
                
                # Assign gradient to the max position in original coordinates
                grads[i, x + max_idx[0], y + max_idx[1]] = output_grad[i, x // pool_width, y // pool_height]
...
def backward_linear(weights, bias, data, output_grad):
    ...
    grad_data =np.dot(weights, output_grad) ## want 50, 1
...
def backward_prop(data, labels, params):
    ...
    y = forward_softmax(logits)
    prob_grad_loss = backward_cross_entropy_loss(y, labels)
    backward_grad_softmax = backward_softmax(logits, prob_grad_loss)
    backward_grad_linear = backward_linear(W2, b2, flattened, backward_grad_softmax)
    grad_data = backward_grad_linear[2].reshape(2, 5, 5)
    backward_grad_relu = backward_relu(first_max_pool, grad_data)
    backward_grad_maxpooliong = backward_max_pool(first_convolution, MAX_POOL_SIZE, MAX_POOL_SIZE, backward_grad_relu)
